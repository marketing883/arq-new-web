...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message: "I apologize, but I'm having trouble connecting right now. Please try again in a moment, or contact our team directly at hello@thearq.ai.",
    });
  }
}
...
function getFallbackResponse(messages: { role: string; content: string }[]): string {
  ...
  if (lastMessage.includes('security') || lastMessage.includes('compliance') || lastMessage.includes('soc')) {
  ...
  if (lastMessage.includes('deploy') || lastMessage.includes('timeline') || lastMessage.includes('how long')) {
  ...
  if (lastMessage.includes('price') || lastMessage.includes('pricing') || lastMessage.includes('cost')) {
  ...
  if (lastMessage.includes('competitor') || lastMessage.includes('vs') || lastMessage.includes('compare')) {
  ...
  if (lastMessage.includes('customer') || lastMessage.includes('case study') || lastMessage.includes('example')) {
